...
def stageOneThree():
    ...
    hex_positions[21][2] = checkYordle()
...
def cycleBench():
    ...
    for i in range(9):
        ...
        if currentYordle > 1:
            swapYordles(currentYordle, i, bench_positions)

        else:
            pyautogui.moveTo(x=bench_positions[i][0], y=bench_positions[i][1], duration=0.2)
            pyautogui.mouseDown()
            pyautogui.moveTo(x=565, y=1000, duration=0.2)
            pyautogui.mouseUp()
